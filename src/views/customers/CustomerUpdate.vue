		...
		<div class="content-wrapper">
			...
			<div class="row">
							...
							<div class="clearfix">
								...
								<div class="card-body" v-if="customer">
									...
									<div class="form-group row">
										...
										<div class="col-sm-9">
											<input type="date" class="form-control" id="birth" v-model="customer.date_of_birth" />
										</div>
									</div>
									<div class="form-group row">
										<label class="col-sm-3 col-form-label">Loại khách hàng</label>
										<div class="col-sm-9">
											<select class="form-control" v-if="typeCustomer" v-model="customer.type_of_customer_id">
												<option
									...
									<div class="form-group row">
										<label for="exampleInputEmail2" class="col-sm-3 col-form-label">Sản phẩm quan tâm</label>
										<div class="col-sm-9">
				...
				<div class="col-md-6 grid-margin stretch-card">
                    ...
                    <button class="btn btn-sm btn-gradient-success" @click="addPhone()">Thêm</button>
                    <div class="list-phones">
                      <div class="phone" v-for="(phone, index) in customer.customer_phone" :key="index">
                        <span> {{ phone.phone }} </span>
                        <button type="button" class="btn-close" @click="del(index)"><i class="mdi mdi-close"></i></button>
                      </div>
                    </div>
									</div>
									<div class="form-group row">
										<label for="text" class="col-sm-3 col-form-label">Zalo:</label>
										<div class="col-sm-9">
											<input
												type="text"
												class="form-control"
									...
									<div class="form-group row">
										<label for="text" class="col-sm-3 col-form-label">Facebook:</label>
										<div class="col-sm-9">
											<input
												type="text"
												class="form-control"
...
<script>
	...
	export default {
      ...
      isNumber: function(evt) {
        ...
        if(charCode == 13){
          this.addPhone();
        }else if ((charCode > 31 && (charCode < 48 || charCode > 57)) && charCode !== 43) {
          evt.preventDefault();;
        } else {
          return true;
        }
      ...
      addPhone(){
        if( this.phone != ''){
          if(this.customer.customer_phone.findIndex(x => x.phone == this.phone) >= 0){
            cuteAlert({
							type: "warning",
            ...
            this.customer.customer_phone.push({ 'phone': this.phone, 'customer_id': this.customer.id});
            this.phone = '';
          }
